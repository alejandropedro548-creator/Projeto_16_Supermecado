...
for i in range(100):
    ...
    total_passes = random.randint(300, 2000)

    passes_certos = int(total_passes * random.uniform(0.70, 0.97))

    nota = round(random.uniform(6.0, 9.8), 1)

    fadiga = random.randint(10, 95)
    ...
    jogadores.append(
        {
            ...
            "Posição": posicao,
            "Pé": random.choice(pes),
            "Altura": random.randint(168, 198),
            "Peso": random.randint(65, 95),
            "Jogos": random.randint(5, 40),
            "Minutos": random.randint(300, 3500),
            "Gols": gols,
            "Assistências": assistencias,
            "Passes Certos": passes_certos,
            "Passes Errados": total_passes - passes_certos,
            "Precisão de Passe": round(passes_certos / total_passes * 100, 1),
            "Desarmes": random.randint(5, 120),
            "Km Percorridos": round(random.uniform(7.5, 13), 1),
            "Sprints": random.randint(20, 250),
            "Velocidade Máxima": round(random.uniform(28, 37), 1),
            "Nota": nota,
            "Fadiga": fadiga,
            "Risco de Lesão": risco,
            "Valor de Mercado": round(random.uniform(1, 100), 1),
            "Potencial": random.randint(65, 99),
        }
    )
...
df["Índice de Desempenho"] = round(df["Índice de Desempenho"], 1)
...
if pagina == "🏠 Início":
    ...
    with c1:
        criar_card("👥 Jogadores", len(df))

    with c2:
        criar_card("⚽ Gols", int(df["Gols"].sum()))

    with c3:
        criar_card("🎯 Assistências", int(df["Assistências"].sum()))

    with c4:
        criar_card("⭐ Nota Média", round(df["Nota"].mean(), 2))
    ...
    st.subheader("🔄 Como os dados circulam no clube")

    fluxo = st.columns(5)

    profissionais = [
        ("👀", "Olheiro", "Coleta informações"),
        ("📊", "Analista", "Analisa dados"),
        ("👨‍🏫", "Técnico", "Toma decisões"),
        ("🏃", "Preparador", "Controla carga"),
        ("⚽", "Jogador", "Evolui"),
    ]

    for coluna, item in zip(fluxo, profissionais):
        with coluna:
            st.markdown(
                f"""
...
# ============================================================
    ...
    with filtro1:
        clube = st.selectbox(
            "🏟️ Clube", ["Todos"] + sorted(df["Clube"].unique().tolist())
        )

    with filtro2:
        posicao = st.selectbox(
            "⚽ Posição", ["Todas"] + sorted(df["Posição"].unique().tolist())
        )

    with filtro3:
        risco = st.selectbox(
            "🩺 Risco de lesão",
            ["Todos"] + sorted(df["Risco de Lesão"].unique().tolist()),
        )
    ...
    if risco != "Todos":
        dados = dados[dados["Risco de Lesão"] == risco]
    ...
    with a:
        criar_card("👥 Atletas", len(dados))

    with b:
        criar_card("⚽ Gols", int(dados["Gols"].sum()))

    with c:
        criar_card("🎯 Assistências", int(dados["Assistências"].sum()))

    with d:
        criar_card("⭐ Nota média", round(dados["Nota"].mean(), 2))

    with e:
        criar_card(
            "🔥 Índice médio", round(dados["Índice de Desempenho"].mean(), 1)
        )
    ...
    with col1:
        gols = dados.groupby("Posição")["Gols"].sum().reset_index()
    ...
    with col2:
        media = dados.groupby("Posição")["Nota"].mean().reset_index()
    ...
    with col4:
        risco_df = dados["Risco de Lesão"].value_counts().reset_index()
    ...
    ranking = dados.sort_values(
        "Índice de Desempenho", ascending=False
    ).head(15)
...
# ============================================================
    ...
    with filtro1:
        idade_max = st.slider("🎂 Idade máxima", 17, 36, 23)

    with filtro2:
        potencial_min = st.slider("⭐ Potencial mínimo", 65, 99, 80)

    with filtro3:
        valor_max = st.slider(
            "💰 Valor máximo (milhões R$)", 1.0, 100.0, 50.0
        )
    ...
    with a:
        criar_card("🔎 Encontrados", len(jogadores_observados))

    with b:
        criar_card(
            "⭐ Potencial médio",
            round(jogadores_observados["Potencial"].mean(), 1)
    ...
    with c:
        criar_card(
            "⚽ Gols",
            int(jogadores_observados["Gols"].sum())
            if len(jogadores_observados) > 0
            else 0,
        )
    ...
    with d:
        criar_card(
            "💰 Valor médio",
            round(jogadores_observados["Valor de Mercado"].mean(), 1)
            if len(jogadores_observados) > 0
            else 0,
        )
    ...
    st.subheader("🎯 Recomendações de contratação")

    if len(jogadores_observados) > 0:
        recomendados = jogadores_observados.sort_values(
            ["Potencial", "Índice de Desempenho"], ascending=False
        ).head(10)
    ...
    st.subheader("📊 Distribuição dos talentos encontrados")

    if len(jogadores_observados) > 0:
        posicao_talentos = (
            jogadores_observados.groupby("Posição")
            .size()
    ...
    st.subheader("📝 Relatório de observação")

    if len(jogadores_observados) > 0:
        escolhido = st.selectbox(
            "Selecione um atleta", jogadores_observados["Jogador"]
        )

        atleta = jogadores_observados[
            jogadores_observados["Jogador"] == escolhido
        ].iloc[0]
...
# ============================================================
    ...
    st.subheader("🔍 Comparação entre atletas")

    jogadores_lista = sorted(df["Jogador"].unique().tolist())

    jogador1, jogador2 = st.columns(2)

    with jogador1:
        atleta1 = st.selectbox(
            "Primeiro jogador", jogadores_lista, key="jogador_1"
        )

    with jogador2:
        atleta2 = st.selectbox(
            "Segundo jogador", jogadores_lista, index=1, key="jogador_2"
        )

    dados1 = df[df["Jogador"] == atleta1].iloc[0]

    dados2 = df[df["Jogador"] == atleta2].iloc[0]
    ...
    st.subheader("📈 Perfil dos jogadores")
    ...
    fig.add_trace(
        go.Scatterpolar(
            r=[dados1[c] for c in categorias],
            theta=categorias,
            fill="toself",
            name=atleta1,
        )
    ...
    fig.add_trace(
        go.Scatterpolar(
            r=[dados2[c] for c in categorias],
            theta=categorias,
            fill="toself",
            name=atleta2,
        )
    ...
    if dados1["Índice de Desempenho"] > dados2["Índice de Desempenho"]:
        ...
        diferenca = (
            dados1["Índice de Desempenho"] - dados2["Índice de Desempenho"]
        )
        ...
        diferenca = (
            dados2["Índice de Desempenho"] - dados1["Índice de Desempenho"]
        )
    ...
    escolhido_nome = st.selectbox(
        "Escolha um jogador para relatório", jogadores_lista, key="relatorio"
    )

    jogador = df[df["Jogador"] == escolhido_nome].iloc[0]
    ...
    if jogador["Nota"] >= 8:
        pontos.append("Excelente avaliação média")

    else:
        melhorias.append("Melhorar consistência técnica")

    if jogador["Precisão de Passe"] >= 85:
        pontos.append("Boa qualidade de passe")

    else:
        melhorias.append("Aumentar precisão dos passes")

    if jogador["Velocidade Máxima"] >= 34:
        pontos.append("Boa capacidade física")

    else:
        melhorias.append("Evoluir velocidade e explosão")

    if jogador["Risco de Lesão"] == "Alto":
        melhorias.append("Necessita controle de carga física")
...
# ============================================================
    ...
    def gerar_movimentacao(posicao):
        ...
        if posicao == "Goleiro":
            x = np.random.normal(10, 8, quantidade)

            y = np.random.normal(34, 12, quantidade)

        elif posicao in ["Zagueiro", "Lateral"]:
            x = np.random.normal(35, 18, quantidade)

            y = np.random.normal(34, 20, quantidade)

        elif posicao in ["Volante", "Meia"]:
            x = np.random.normal(55, 20, quantidade)

            y = np.random.normal(34, 22, quantidade)

        else:
            x = np.random.normal(75, 18, quantidade)
    ...
    jogador_nome = st.selectbox(
        "⚽ Escolha o jogador", sorted(df["Jogador"].tolist())
    )

    jogador = df[df["Jogador"] == jogador_nome].iloc[0]

    x, y = gerar_movimentacao(jogador["Posição"])
    ...
    fig.update_layout(
        title=f"🔥 Movimentação: {jogador_nome}",
        template="plotly_dark",
        height=600,
        xaxis=dict(range=[0, 100], showgrid=False),
        yaxis=dict(range=[0, 68], showgrid=False),
    )
    ...
    st.subheader("📊 Análise da movimentação")

    c1, c2, c3, c4 = st.columns(4)

    with c1:
        criar_card("Posição", jogador["Posição"])

    with c2:
        criar_card("Distância", f"{jogador['Km Percorridos']} km")

    with c3:
        criar_card("Sprints", jogador["Sprints"])

    with c4:
        criar_card("Velocidade", f"{jogador['Velocidade Máxima']} km/h")
...
# ============================================================
    ...
    st.success(f"Estratégia selecionada: {esquema}")
    ...
    st.subheader("⭐ Sugestão de escalação baseada em dados")

    escalação = []

    for pos in posicoes:
        jogador_pos = df[df["Posição"] == pos]
    ...
    escala_df = pd.DataFrame(escalação)

    st.dataframe(escala_df, use_container_width=True, hide_index=True)
    ...
    st.subheader("📈 Visão coletiva do elenco")

    c1, c2, c3 = st.columns(3)

    with c1:
        criar_card("⭐ Média técnica", round(df["Nota"].mean(), 2))

    with c2:
        criar_card("⚽ Média gols/jogo", round(df["Gols"].mean(), 2))

    with c3:
        criar_card(
            "🏃 Intensidade média", round(df["Km Percorridos"].mean(), 1)
        )
...
# ============================================================
    ...
    with a:
        criar_card("🏃 Km médio", round(df["Km Percorridos"].mean(), 1))

    with b:
        criar_card("🔥 Sprints médios", round(df["Sprints"].mean(), 1))

    with c:
        criar_card("⚡ Velocidade", round(df["Velocidade Máxima"].mean(), 1))

    with d:
        criar_card(
            "🩺 Alto risco", len(df[df["Risco de Lesão"] == "Alto"])
        )
    ...
    st.subheader("🚨 Atletas que precisam de atenção")

    risco_alto = df[df["Risco de Lesão"] == "Alto"].sort_values(
        "Fadiga", ascending=False
    )
    ...
    carga = df.sort_values("Fadiga", ascending=False).head(15)
...
# ============================================================
    ...
    jogador_escolhido = st.selectbox(
        "Selecione o atleta", sorted(df["Jogador"].tolist())
    )

    atleta = df[df["Jogador"] == jogador_escolhido].iloc[0]
    ...
    with c1:
        criar_card("⭐ Nota", atleta["Nota"])

    with c2:
        criar_card("⚽ Gols", atleta["Gols"])

    with c3:
        criar_card("🎯 Assistências", atleta["Assistências"])

    with c4:
        criar_card("📈 Índice", atleta["Índice de Desempenho"])
    ...
    if atleta["Nota"] >= 8:
        st.success("Atleta apresenta ótimo desempenho técnico.")

    else:
        st.warning("Atleta deve buscar evolução técnica.")
...
# ============================================================
    ...
    for coluna, item in zip(colunas, integrantes):
        with coluna:
            st.markdown(
                f"""
            <div class="card" style="text-align:center">
